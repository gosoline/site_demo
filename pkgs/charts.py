...
mycolors = ('#2A579A', '#FF7F0E', '#227447', '#D62728', '#9467BD', '#8C564B',
            '#E377C2', '#808080')
...
"""
    # 初始化绘图网格和折线绘图区
    chart_grid = Grid(init_opts=opts.InitOpts(
        # 关闭动画效果
        # animation_opts=opts.AnimationOpts(animation=False),
    ))
    chart_bar = Bar().add_xaxis(data[data.columns[0]].to_list()).add_yaxis(
        str(data.columns[1]), data[data.columns[1]].to_list()).set_global_opts(
            xaxis_opts=opts.AxisOpts(name=str(data.columns[0]),
                                     axislabel_opts=opts.LabelOpts(
                                         rotate=-15, font_size=10)),
...
def line_json(data: DataFrame,
    ...
    if not data.empty:
        ...
        chart_line = Line(init_opts=opts.InitOpts(
            # 关闭动画效果
            animation_opts=opts.AnimationOpts(animation=False), )).add_xaxis(
                # x轴数据导入
                xdata)
        # 记录绘制曲线数，用于最后调整图表样式
        line_count = 0
        ...
        for i in range(min(len(data.columns), 8)):
            ...
            if not isinstance(data[data.columns[i]][0], np.number):
                continue
            # 取出数据并取四位小数
            y_data = data[data.columns[i]].to_numpy().astype(float).round(
                4).tolist()
            # 设置y轴样式
            chart_line = chart_line.extend_axis(yaxis=opts.AxisOpts(
                # 奇数Y轴在左侧，偶数在右侧
